...
public class SkinFixer {
	 ...
	 * 2 = new resources<br>
	 */
	public static byte Resource_MODE = 0;
	...
	public static void runSkinFixer(String connectionId, HttpExchange exchange, String hostname, String url) throws IOException {
		...
		logger.log("[Skin And Resources Fixer] " + connectionId + "Request served successfully. HTTP Connection to game closed.");
	}
	...
	private static byte[] doRunSkinFixer(String connectionId, String hostname, String url) throws IOException {
		...
		if(hostname.equals("www.minecraft.net") && url.startsWith("/skin/")) {
			...
			if(MinecraftFandomWikiIsFuckers.equals(Minecraft_sOldestSkinApplyVersinIs_1_1_2_01)) {
				logger.log("[Skin And Resources Fixer] " + connectionId + "Username changing is required!!");
				...
				logger.log("[Skin And Resources Fixer] " + connectionId + "Type your original username.");
			...
			logger.log("[Skin And Resources Fixer] " + connectionId + "Received Generation 1 request for skin of player " + username);
			...
			textureType = "SKIN";
			autoWouldFlipBottoms = true;
			logger.log("[Skin And Resources Fixer] " + connectionId + "Received Generation 2 request for skin of player " + username);
			...
			textureType = "CAPE";
			autoWouldFlipBottoms = true;
			logger.log("[Skin And Resources Fixer] " + connectionId + "Received Generation 2 request for cape of player " + username);
		} else if(hostname.equals("skins.minecraft.net") && url.startsWith("/MinecraftSkins/")) {
			username = url.substring("/MinecraftSkins/".length(), url.length() - ".png".length());
			textureType = "SKIN";
			autoWouldFlipBottoms = false;
			logger.log("[Skin And Resources Fixer] " + connectionId + "Received Generation 3 request for skin of player " + username);
		} else if(hostname.equals("skins.minecraft.net") && url.startsWith("/MinecraftCloaks/")) {
			username = url.substring("/MinecraftCloaks/".length(), url.length() - ".png".length());
			textureType = "CAPE";
			autoWouldFlipBottoms = false;
			logger.log("[Skin And Resources Fixer] " + connectionId + "Received Generation 3 request for cape of player " + username);
		} else if(hostname.equals("www.minecraft.net") && url.startsWith("/skin/")){
			username = ("0");
			textureType = "OldResources";
			autoWouldFlipBottoms = false;
			logger.log("[Skin And Resources Fixer] " + connectionId + "Received Generation 1 request for resources ");
		} else if(hostname.equals("s3.amazonaws.com") && url.startsWith("/MinecraftResources/")){
			username = ("0");
			textureType = "NewResources";
			autoWouldFlipBottoms = false;
			logger.log("[Skin And Resources Fixer] " + connectionId + "Received Generation 2 request for resources ");
		} else {
		...
		if(uuid.equals("3f92defab3ec4d0389d6a0abd4a6ed01")){
			...
			return new byte[0];
		} else if(textureType.equals("OldResources")){
			logger.log("[Skin And Resources Fixer] " + connectionId + "Old Resource Loading");
			Resource_MODE = 1;
			
			String textureUrl = "0";
			
			return getRemoteFile(textureUrl, connectionId);
			//URL resourcesUrl = SkinFixer.Class().getClassLoader().getResource (resources);
			//
			//return getRemoteFile(resourcesUrl, connectionId);
		} else if(textureType.equals("NewResources")){
			logger.log("[Skin And Resources Fixer] " + connectionId + "New Resource Loading");
			Resource_MODE = 2;
			
			String textureUrl = "0";
			
			return getRemoteFile(textureUrl, connectionId);
			//URL resourcesUrl = SkinFixer.Class().getClassLoader().getResource (MinecraftResources);
			//
			//return getRemoteFile(resourcesUrl, connectionId);
		} else{
			logger.log("[Skin And Resources Fixer] " + connectionId + "UUID of player " + username + " is " + uuid);
			...
			if(textureProperties == null) {
				logger.log("[Skin And Resources Fixer] " + connectionId + "Player does not have texture properties.");
				return new byte[0];
			}
			...
			if(!(texProps.has("textures") && texProps.get("textures") instanceof JSONObject)) {
				logger.log("[Skin And Resources Fixer] " + connectionId + "Texture properties are missing the textures object.");
				return new byte[0];
			}
			...
			if(!(textures.has(textureType) && textures.get(textureType) instanceof JSONObject)) {
				logger.log("[Skin And Resources Fixer] " + connectionId + "Player doesn't have a " + textureType.toLowerCase());
				return new byte[0];
			}
			...
			logger.log("[Skin And Resources Fixer] " + connectionId + "ID of " + textureType.toLowerCase() + " texture is " + textureId);
	...
	private static byte[] getRemoteFile(String url, String connectionId) throws IOException {
		if(remoteFileCache.containsKey(url)) {
			...
			//baos.close();
			//logger.log("[Loading] " + connectionId + "Successfully loaded (" + file.length + " bytes)");
			//remoteFileCache.put(url, file);
			//return file;
			
			//ByteArrayOutputStream baos = new ByteArrayOutputStream(4096);
			//try (var in = SkinFixer.class.getResource("/" + resources)) {
			//    in.transferTo(baos);
			//}
			//
			//byte[] file = baos.toByteArray();
			...
			ByteArrayOutputStream baos = new ByteArrayOutputStream(4096);
			InputStream is = ResourcesFixer.accessFile(Resource_MODE);
			...
			//ByteArrayOutputStream baos = new ByteArrayOutputStream(180224);
			//try (var in = SkinFixer.class.getResource("/" + MinecraftResources)) {
			//    in.transferTo(baos);
			//}
			//
			//byte[] file = baos.toByteArray();
